...
const char *SQLiteWrapper::errMesg() { return sqlite3_errmsg(db); }
...
int SQLiteWrapper::step() const { return sqlite3_step(stmt) == SQLITE_ROW; }

void SQLiteWrapper::final() const { sqlite3_finalize(stmt); }
...
void SQLiteWrapper::sortBy(int where_col, std::string val, int order_col,
  ...
  std::cout << where_col << " " << val << " " << order_col << "\n";
...
void SQLiteWrapper::setColNames() {
  ...
  for (int i = 0; step();) {
    col_name_vec.push_back(getColText(1));
  }
