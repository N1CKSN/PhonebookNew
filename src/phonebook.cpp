...
#define DB_DIR "/home/nix/code/PHONEBOOK-master/data/sqlitedb.db"
...
int main() {
  ...
  try {
    ...
    Fl_Window *win =
        new Fl_Window(win_x, win_y, win_w, win_h, "Телефонный справочник");

    DataTable *table = new DataTable(table_x, table_y, table_w, table_h, sw);
    ...
    TableInputSort *inp_house_n =
        new TableInputSort(*table, big_button_w + spacing_x * 2, button_h * 2,
    ...
    std::vector<Fl_Input *> inp_vec = {inp_p_number, inp_full_name, inp_street,
                                       inp_house_n, inp_flat};

    ClearInputButton *cbut =
        new ClearInputButton(inp_vec, table, big_button_w + spacing_x * 3,
                             button_h + button_h / 2 - 3, small_buttow_w / 1.5,
                             button_h, "Очистить ввод");

    InsertButton *ibut = new InsertButton(
        table, big_button_w + small_buttow_w / 1.5 + spacing_x * 4,
        button_h + button_h / 2 - 3, small_buttow_w / 2, button_h, "Добавить");

    DeleteButton *dbut = new DeleteButton(
        table, big_button_w + (small_buttow_w / 1.5) * 1.7 + spacing_x * 5.5,
        button_h + button_h / 2 - 3, small_buttow_w / 2, button_h, "Удалить");

    UpdateButton *ubut = new UpdateButton(
        table, big_button_w + (small_buttow_w / 1.5) * 2.4 + spacing_x * 7,
        button_h + button_h / 2 - 3, small_buttow_w / 2, button_h, "Изменить");
